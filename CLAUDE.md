...
- 一次 Vision 调用同时完成"看图分析"和"画像素"：返回一个固定角色的调色板（结构见下方"调色板结构固定"一条）+ 11 个必画面（head 6 个面 + body_front + 双臂双腿的正面共 4 个）的调色板索引网格 + AI 自主追加的最多 4 个"特色面"；不再有独立的顶层颜色字段，所有颜色都通过调色板索引引用
...
- 除了固定的 11 个必画面（head 6 个面 + body_front + 胳膊正面×2 + 腿正面×2），AI 可以自主追加最多 4 个"特色面"（比如背后徽标、袖子图案），用同样的索引格式，判断标准写在 prompt 里；未被追加的面照常走 `procedural.py`

### Procedural Fill (`app/services/procedural.py`)
除了 head 6 个面、body_front 和四个肢体正面（共 11 个必画面，以及 AI 自主追加的特色面），其余基础区域（body 的 back/top/bottom/left/right、双臂各自的 back/top/bottom/left/right、双腿各自的 back/top/bottom/left/right）**不经过 AI**，由 `generate_procedural_regions` 程序化生成，不再做任何合成阴影（Minecraft 自己的光照已经会给 3D 模型加阴影）：
  - back/left/right 用 `propagate_front_row` 直接复制同一部位正面网格里每一行的颜色（取该行中间列），所以正面上一条衣服边界（比如袖子在半路结束）沿着肢体包裹一圈后依然连贯
  - top/bottom 这两个小端面用命名颜色做纯色填充（腿的 bottom 用 `shoe_color`，其余用该部位的 `_main` 色）
  左右两侧直接复用同一套规则（不需要真正的像素镜像，因为规则本身就是对称的）；`exclude_keys` 参数让 AI 自主画的特色面不被程序化覆盖。这是 token 优化的核心：AI 逐像素生成的区域从 ~4096 降到 11 个必画面（约 480+ 像素，视手臂宽度而定），配合合并为单次调用，单次运行 token 消耗大幅下降。
...
- **绝大部分程序化生成**: 只有 11 个必画面（head 6 个面 + body_front + 胳膊/腿正面×4）由 AI 逐像素生成，其余全部由 `procedural.py` 用 `propagate_front_row`（back/left/right 复制正面每行颜色）+ 端面纯色填充规则生成，这是 token 消耗的主要优化点
